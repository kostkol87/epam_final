...
<div class="top-menu">
    ...
    <h2 id="status"><fmt:message
            key="Hello"/>, ${pageContext.session.getAttribute('user').getName()} ${pageContext.session.getAttribute('user').getSurname()} </h2>
...
<div id="add-dir">
    <button id="small-btn" onclick="window.location='/jsp/showOrders.jsp'"><fmt:message key="MyOrders"/></button>
    <br>
    ...
    <c:if test="${role == 3}">
        <form action="/addDirection">
            <table border="2">
                ...
                <tr>
                    <td><input type="text" name="fieldDeparture" required
                               placeholder="<fmt:message key="Departure"/>"></td>
                    <td><input type="datetime-local" name="depTime" required></td>
                    <td><input type="text" name="fieldDestination" required
                               placeholder="<fmt:message key="Destination"/>"></td>
                    ...
                    <td><input type="text" name="capacity" required placeholder="<fmt:message key="Capacity"/>">
                    </td>
...
<div>
    <zz:directions admin="${pageContext.session.getAttribute('user').getRole()}"
                   page="${pageContext.session.getAttribute('page')}"
                   locale="${pageContext.session.getAttribute('locale')}"/><br>
</div>

<div id="paginator">
    <a href="/page?fwd=l&target=${pageContext.request.servletPath}"> << </a>
    ...${pageContext.session.getAttribute('page')}... <a
        href="/page?fwd=r&target=${pageContext.request.servletPath}"> >> </a><br>
</div>
